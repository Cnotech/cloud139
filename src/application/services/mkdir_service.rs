...
pub async fn mkdir(
    ...
    match storage_type {
        StorageType::PersonalNew => mkdir_personal(config, &name, &parent, force).await?,
        StorageType::Family => mkdir_family(config, &name, &parent).await?,
        StorageType::Group => mkdir_group(config, &name, &parent).await?,
    }
    Ok(())
}
