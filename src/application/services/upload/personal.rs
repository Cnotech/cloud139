    ...
    if let Some(part_infos_response) = data.part_infos {
        if part_infos_response.is_empty() {
            ...
            upload_parts(UploadPartsParams {
                ...
                part_size: crate::application::services::upload_service::get_part_size(
                    file_size,
...
async fn init_upload(
    ...
    let part_size =
        crate::application::services::upload_service::get_part_size(file_size, config.custom_upload_part_size);
...
pub(super) async fn upload_parts(params: UploadPartsParams<'_>) -> Result<(), ClientError> {
    ...
    let upload_urls = crate::application::services::upload::personal_parts::get_upload_urls(
    ...
    for i in 0..part_count {
        ...
        crate::application::services::upload::personal_parts::upload_single_part(
            &upload_urls,
            part_number,
            &buffer[..bytes_read],
            params.pb.clone(),
    ...
    crate::application::services::upload::personal_parts::confirm_upload(config, host, file_id, upload_id, content_hash).await
}
