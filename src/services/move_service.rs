...
pub async fn mv(
    ...
    match storage_type {
        StorageType::PersonalNew => mv_personal(config, sources, target, force).await?,
        StorageType::Family => mv_family(config, sources, target).await?,
        StorageType::Group => mv_group(config, sources, target).await?,
    }
    Ok(())
}

async fn mv_personal(
    config: &crate::config::Config,
    sources: &[String],
    target: &str,
    force: bool,
...
async fn mv_family(
    ...
    let resp: serde_json::Value = client
        .isbo_post("/isbo/openApi/createBatchOprTask", body)
        .await?;
