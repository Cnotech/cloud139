...
pub async fn get_group_disk_info(config: &Config) -> Result<crate::models::GroupDiskInfoResp, ClientError> {
    ...
    let result: crate::models::GroupDiskInfoResp = resp.json().await?;
    Ok(result)
}
